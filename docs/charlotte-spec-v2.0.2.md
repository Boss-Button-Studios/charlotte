...
**Version:** 2.0.2
**Status:** Final (result content delivery added; ready for Phase A implementation)
**Supersedes:** `charlotte-spec-v2.0.1.md` (preserved as historical reference). Earlier audit trails at `docs/charlotte-spec-v2.0.md` Appendix A and `docs/charlotte-spec-v2.0.1.md` Appendix A.
...
This document specifies Charlotte v2.0.2 — a feature revision over v2.0.1 adding result content delivery. v2.0.1's destination verifier already fetched the proposed result URL to score it for relevance; v2.0.2 makes those bytes available to the caller as part of the result, rather than discarding them after scoring. The architecture from v2.0/v2.0.1 is otherwise unchanged.

Sections of v1.4 that carry over unchanged remain referenced rather than repeated. The decision log for all three review cycles (v2.0 draft → v2.0 → v2.0.1 → v2.0.2) is in Appendix A.
...
- **G7. Return the deliverable, not always just a pointer to it.** For navigation goals, the deliverable is a URL. For `document_link` goals, the deliverable is the document bytes themselves — fetched during destination verification and made available to the caller without a second fetch. Other goal types may opt in to bytes-with-URL via `fetch_result_content=True`. This closes the practical gap between "find this PDF" returning a URL versus returning the file the caller actually needs.
...
### 7.7 Result content delivery (NEW in v2.0.2)

The destination verifier already fetches the proposed result URL during verification (§7.3). v2.0.1 discarded those bytes after scoring. v2.0.2 makes them available to the caller as part of the result, eliminating the second fetch that callers would otherwise have to perform.

#### 7.7.1 Default behavior by goal type

- **`document_link` goals**: result bytes are included **by default**. The whole purpose of this goal type is to find a downloadable artifact; returning a URL alone forces the caller into a redundant second fetch for content Charlotte has already retrieved.
- **All other goal types** (navigation, phone/date/address/price extraction, freeform fact): result bytes are **NOT** included by default. The caller asked for a URL or a fact, not a payload. Callers wanting bytes for these goal types set `fetch_result_content=True` (§8.1).

Setting `fetch_result_content=False` explicitly suppresses content even for `document_link` goals, for callers who only want the URL.

#### 7.7.2 `ResultContent` schema

```python
@dataclass(frozen=True)
class ResultContent:
    content: bytes | None         # the fetched bytes; None when result_to_file was set
    content_type: str | None      # from Content-Type header, normalized
    content_length: int           # actual bytes captured (always set, even with file delivery)
    suggested_filename: str | None  # from Content-Disposition; falls back to URL path basename
    etag: str | None              # from ETag header, if the server provided one
    fetched_at: datetime          # when verification fetched the bytes
    file_path: Path | None        # set when result_to_file was used; None for inline bytes
```

Either `content` or `file_path` is populated; never both, never neither. `content_length` is the source of truth for size regardless of delivery mechanism.

The fields `content_type`, `suggested_filename`, and `etag` are populated whenever the server provided the corresponding header, even when bytes themselves are not returned (e.g., when `fetch_result_content=False`). This lets callers see metadata about the destination without paying for the bytes.

#### 7.7.3 Size cap

`max_result_bytes` parameter on `crawl()` / `find_link()` caps the size of content captured during verification. Default: 10 MB, matching v1.4's fetcher cap (S-C2).

When a response exceeds the cap:

- If `result_to_file` is set: streaming to disk continues until the cap is reached, then `CharlotteResponseTooLargeError` is raised (consistent with v1.4 fetcher behavior).
- If `result_to_file` is not set: the destination verification fails with the same error. The candidate is recorded in `verified_candidates` with `passed=False, reason="response_too_large"`, and the engine pops the next priority-queue item.

#### 7.7.4 File-based delivery

`result_to_file` parameter (default `None`) accepts a directory path. When set:

- Content is streamed to disk during the verification fetch (no double-buffering).
- A unique filename is chosen using `suggested_filename` if available, falling back to a safe URL-derived name.
- The full on-disk path is returned in `ResultContent.file_path`; `ResultContent.content` is `None`.
- The directory must exist and be writable; Charlotte does not create directories.

File-based delivery is the recommended path for:

- Large documents that don't fit comfortably in memory.
- Callers archiving documents directly (skips the bytes → file round trip).
- The consolidation service shape, where periodic recrawls of dated documents accumulate into a directory of historical versions.

#### 7.7.5 Reuse of the verification fetch

The bytes returned were captured during destination verification — there is no second fetch. This means:

- **No additional cost or latency.** The fetch happened anyway for §7.3 scoring.
- **No TOCTOU window.** The content delivered is exactly what the verifier evaluated. A site rotating content between verify and deliver cannot exploit a gap that doesn't exist.
- **Verification failures suppress content.** When verification fails, no content is returned. The bytes that were fetched are discarded along with the rejected candidate.

This is enabled by a small change to the verifier's internal contract: instead of fetching, scoring, and discarding bytes, the verifier now fetches, scores, and retains bytes when the candidate passes. The retention is bounded by `max_result_bytes` and the verifier's normal lifetime (single crawl).

#### 7.7.6 Streaming events

The new metadata is included in `ResultFound` events as a `ResultContentMetadata` field — content type, length, suggested filename — but **not** the bytes themselves. The bytes appear only in the final `CrawlResult` (or `LinkResult` for `find_link()`).

Rationale: streaming events are for live progress observation. A 5 MB byte payload in an event is a memory and serialization hazard for consumers piping events to logs, dashboards, or queues. Metadata in events; bytes in final results.

#### 7.7.7 What this does not do

- **Authentication.** Same as the rest of Charlotte v2 — unauthenticated content only. A document behind a login wall fails destination verification (login walls are detected per §7.3 `"existence"` mode).
- **Streaming delivery to the caller.** Bytes are buffered in memory or streamed to a file; chunked-streaming-to-caller is deferred to v2.x. The buffered approach is sufficient for documents up to `max_result_bytes`; the file approach is sufficient for larger documents at the cost of disk I/O.
- **In-memory or on-disk caching across crawls.** Each crawl fetches its own bytes. Preserves NG2 (no persistent storage). Cross-crawl caching is the caller's responsibility — the consolidation service's archival directory is the right place for it.
- **Multi-document delivery.** `ResultContent` is per-result-URL. When `max_results > 1`, each entry in `result_urls` gets its own `ResultContent` (or `None` if that entry didn't qualify for content). `find_link()` is single-result so it returns a single `ResultContent | None`.
...
async def crawl(
    ...
    *,
    ...
    # NEW in v2.0.2: result content delivery
    fetch_result_content: bool | None = None,  # None = default per goal type (True for document_link, False otherwise)
    max_result_bytes: int = 10_485_760,        # 10 MB, matching v1.4 fetcher cap
    result_to_file: Path | None = None,        # when set, content is streamed to this directory instead of buffered
...
The v2.0.2 parameters apply equivalently to `find_link()`, which returns a single `LinkResult` with an optional `ResultContent` field (vs. `CrawlResult.result_contents` as a list).

### 8.2 Backward compatibility

Carries over from v2.0 §8.2 unchanged. v1.4-style calls continue to work; v2.0-style and v2.0.1-style calls continue to work. v2.0.2 adds optional parameters with defaults — except that `document_link` goals now return bytes by default, which is a behavior change for any v2.0.1 caller that was relying on `document_link` returning just a URL. The behavior change is intentional (it closes the gap noted in G7) but is documented prominently here for migration awareness.
...
### 9.1 Result content extensions (NEW in v2.0.2)

```python
@dataclass(frozen=True)
class CrawlResult:
    # ...all v1.4, v2.0, and v2.0.1 fields retained...

    # NEW in v2.0.2: per-result content, aligned to result_urls.
    # Each entry corresponds to the same-index entry in result_urls.
    # An individual entry is None when content was not requested for
    # that goal type or fetch_result_content was False.
    result_contents: list[ResultContent | None]


@dataclass(frozen=True)
class LinkResult:  # find_link() return shape
    # ...all v1.4, v2.0, and v2.0.1 fields retained...

    # NEW in v2.0.2: single optional content for the single result.
    result_content: ResultContent | None
```

`result_contents` is always a list of length matching `result_urls` for typing consistency. Entries are individually `None` when content was not requested (per the per-goal-type defaults in §7.7.1 or explicit `fetch_result_content=False`).

When `found=False`, `result_contents` is the empty list `[]`. The `verified_candidates` field still records what was tried; content from failed verifications is discarded (§7.7.5).
...
### 10.5 `ResultFound` content metadata (NEW in v2.0.2)

`ResultFound` events for results that have associated content carry a `content_metadata` field with the content's shape — but **not** the bytes themselves. This lets event consumers see "a 2.3 MB PDF named '2026-directory.pdf' was found" without paying memory or serialization cost for the payload.
...
@dataclass(frozen=True)
class ResultFound:
    # ...all v1.4 fields retained...

    # NEW in v2.0.2: metadata only, no bytes.
    # None when no content was fetched for this result.
    content_metadata: ResultContentMetadata | None
```

The bytes themselves appear only in the final `CrawlResult.result_contents` (or `LinkResult.result_content`), consistent with §7.7.6.
...
**Hidden-region attacks.** Pages may place valid-looking answers in CSS-hidden regions (`display:none`, `visibility:hidden`, `opacity:0`, off-screen positioning), `<meta>` tags, scripts, `<noscript>`, comments, or HTML attributes not normally rendered. **Defense:** v1.4 §9.1 Layer 1 sanitization strips these before content reaches the extractor or ranker — via the `hidden` attribute, inline styles, and flat `<style>` rules (`display:none` / `visibility:hidden`) matched by selector. The sanitizer is a heuristic, not a full CSS engine: hiding nested in at-rules (`@media`), applied by JavaScript at runtime, or via exotic `clip`/`transform` tricks is **not** removed (see SECURITY.md S-L3). It raises the bar against hidden-text injection; the model-output provenance check and the ranker's independent scoring are the defenses that do not depend on perfect hidden-text removal. Sanitizer coverage carries over from v1.4 and is extended in v2.
...
| T-70 | `document_link` returns bytes by default | A `document_link` crawl returns a populated `ResultContent` without setting `fetch_result_content` |
| T-71 | Navigation goals omit bytes by default | A `navigation` crawl returns `result_contents` with `None` entries unless `fetch_result_content=True` |
| T-72 | `fetch_result_content=False` overrides default | Explicit False suppresses content even for `document_link` goals; metadata still populated |
| T-73 | `max_result_bytes` enforced | Oversized response triggers `CharlotteResponseTooLargeError`; candidate recorded as rejected in `verified_candidates` |
| T-74 | `result_to_file` writes to disk | Content streams to the specified directory; `ResultContent.content` is `None` and `file_path` is set |
| T-75 | `suggested_filename` precedence | `Content-Disposition` filename takes precedence over URL-path basename; both produce safe filesystem names |
| T-76 | No double fetch | A document_link crawl issues exactly one network fetch for the verified destination URL (the verifier fetch is reused, not repeated) |

Total v2 test additions: 43 (T-34 through T-76).
...
- `document_link` corpus crawls actually deliver bytes (not just URLs) and the bytes parse as the expected content type.
...
v2.0.2 additions land entirely in Phase C alongside destination verification:

- **§7.7 result content delivery** is implemented as part of the destination verifier (the bytes-retention change is internal to that component). New types (`ResultContent`, `ResultContentMetadata`) and new fields on `CrawlResult`/`LinkResult`/`ResultFound` accompany the verifier's Phase C delivery.
- **`document_link` extractor** in Phase C is the goal-type that activates the new content-by-default behavior; symmetric across phases.

No phase reordering. No additional phases.

---

## 15. Things deliberately not addressed

Carries over from v2.0 §15, with these additions:
...
- **Streaming result content to the caller.** v2.0.2 buffers content in memory (up to `max_result_bytes`) or streams to a file (`result_to_file`). Chunked-streaming back to the caller via an async iterator is deferred to v2.x. Buffered or file-based delivery is sufficient for the consolidation service shape and other current callers; streaming becomes interesting when callers need to start processing content before the full document arrives.
- **Authenticated content fetching.** Same as v2.0.1: Charlotte does not support credential-bearing fetches. The `document_link` content-delivery path inherits this limitation — unauthenticated documents only. Authenticated content remains the caller's responsibility (fetch separately with credentials, or pre-authenticate via a proxy).
...
- **Result content** (NEW in v2.0.2) — the bytes of a verified destination, captured during destination verification and returned to the caller via `ResultContent`. Default-on for `document_link` goals; opt-in for other goal types via `fetch_result_content=True`.
- **Verifier fetch reuse** (NEW in v2.0.2) — the property that result content is captured during the fetch that destination verification performed for scoring; no second network fetch occurs.
...
| 2.0.2 | v2.0.2 | Result content delivery: `document_link` goals return bytes by default; opt-in for other goal types via `fetch_result_content`; file-based delivery (`result_to_file`) for large documents; verifier fetch reused with no extra network call; `ResultContentMetadata` on `ResultFound` events |

---

## Appendix A — Decision log

Decisions made across all three review cycles, in section order. Earlier rows from prior reviews preserved verbatim; new rows from each review marked with their origin.
...
| §2.1 (v2.0.2) | Add "return the deliverable" as a goal? | Yes — G7 | Closes the gap between "find this PDF" returning a URL and returning the file itself; v1.4/v2.0/v2.0.1 had this implicit but unstated. |
| §7.7 (v2.0.2) | Default content delivery per goal type? | `document_link`: on; others: opt-in via `fetch_result_content=True` | `document_link` is the goal type whose purpose is a downloadable artifact; navigation/fact types don't need a payload by default. |
| §7.7 (v2.0.2) | Delivery format? | Inline bytes default; `result_to_file` opt-in for large documents | Inline is simplest for typical PDF sizes; file path keeps memory bounded for unusual cases without forcing async streaming complexity into v2.0. |
| §7.7 (v2.0.2) | Reuse verifier fetch? | Yes — bytes captured during verification, retained on success, discarded on failure | No additional fetch cost; eliminates TOCTOU window between verify and deliver. |
| §7.7 (v2.0.2) | Streaming chunked delivery to caller? | Deferred to v2.x | Buffered + file-based delivery suffices for current use cases (consolidation service archival); streaming complexity not warranted until concrete demand. |
| §7.7 (v2.0.2) | Authenticated content fetching? | Deferred — out of scope, same as v2.0.1 | Inherits the v2 architecture's no-auth limitation. |
| §10.5 (v2.0.2) | Bytes in `ResultFound` events? | No — metadata only (`ResultContentMetadata`); bytes only in final `CrawlResult` | Streaming events are for live progress; multi-MB payloads in events are a memory/serialization hazard for consumers piping events to logs or queues. |
