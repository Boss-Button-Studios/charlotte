...
Covers all three flag conditions (instruction-mirroring, confidence spike on
thin content, zero-links/no-path), the happy path, multi-flag cases, and the
T-24 injection scenario.
"""
...
def test_clean_decision_passes():
    """A well-formed decision with unvisited links passes all checks."""
    result = check_plausibility(_decision(), _RICH_TEXT, _VISITED)
...
def test_found_true_with_result_url_passes():
    ...
    result = check_plausibility(d, _RICH_TEXT, _VISITED)
    assert result.passed is True
...
def test_back_link_to_visited_page_does_not_flag():
    ...
    result = check_plausibility(d, _RICH_TEXT, visited)
    assert not any(f.name == "already_visited_link" for f in result.flags)
    assert result.passed is True


# ---------------------------------------------------------------------------
# Flag 1 — Instruction mirroring (covers T-24)
# ---------------------------------------------------------------------------
...
def test_instruction_mirroring_ignore_your_goal():
...
# ---------------------------------------------------------------------------
# Flag 2 — Confidence spike on thin content
# ---------------------------------------------------------------------------
...
def test_confidence_spike_on_thin_content_triggers_flag():
    ...
    result = check_plausibility(d, _THIN_TEXT, _VISITED)
    assert result.passed is False
...
def test_multiple_flags_all_reported():
    ...
    d = _decision(
        ...
        links_to_follow=["https://example.com/x"],
...
def test_inner_charlotte_internal_error_reraises_unchanged():
    ...
    with patch(
        "charlotte.core.plausibility._check_instruction_mirroring", side_effect=inner
    ):
        with pytest.raises(CharlotteInternalError) as exc_info:
            check_plausibility(_decision(), _RICH_TEXT, _VISITED)
...
def test_unexpected_exception_wrapped_as_internal_error():
    ...
    with patch(
        "charlotte.core.plausibility._check_instruction_mirroring",
